...
async def main():
    ...
    model = QNet(state_dim=8, num_actions=4, atoms=atoms, gamma=0.99, double=True, tau=5)
    ...
    for i in range(config.episodes):
        result = run_episode(env, model, eps=0.0, max_episode_steps=config.max_steps)
    ...
    if config.demo:
        ...
        result = run_episode(demo_env, model, config.max_steps, eps=0.0)
